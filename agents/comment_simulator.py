...
═══ 4 类读者身份(必须覆盖至少 3 类)═══
  ...
  · 追读派(主线党):关心剧情推进、情感投入。常说"催更""主角太惨了""快点反击啊"。
     sentiment 多为 positive。
  · 挑刺派(逻辑党):挑设定漏洞、文笔毛病、节奏拖沓、人物 OOC。
     sentiment 多为 critical(批评但不恶毒)。
  · 路过派(吐槽党):随口调侃、玩梗、不深入。"哈哈这章看着乐""xxxx 这反派演技不错"。
     sentiment 多为 neutral。
  · 章评党(金句党):截取本章金句/好画面,夸具体段落,或骂某句出戏。
     sentiment 可 positive 或 negative。
...
_ALLOWED_TYPES = {"追读派", "挑刺派", "路过派", "章评党"}

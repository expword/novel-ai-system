...
def plan_book_structure_only(state: NovelState) -> None:
    ...
    concept_block = format_concept_brief(state)
    intent_brief = build_intent_brief(state, "volume_planner")
...
def _plan_outlines_for_indices(state, volume_index, vol, indices, prior_outlines, chars_str, lines_str, stage=None):
    ...
    # ── P1-1: outline.goal 具体度校验 ──────────────────
    # goal 抽象("推进剧情"/"巩固关系"/"深入了解"等) 会让下游 chapter_planner 拿到
    # 抽象目标 → 写出抽象 scene_beats → writer 写出抽象正文。源头拦截。
    try:
        from persistence.checkpoint import add_progress_warning
        for o in new_outlines:
            ch_idx = o.get("index", 0)
            goal_text = (o.get("goal") or "").strip()
            issue = _check_outline_goal_concreteness(goal_text)
            if issue:
                add_progress_warning(
                    level="warn",
                    source=f"outline:V{volume_index}Ch{ch_idx}.concreteness",
                    message=(
                        f"第 {volume_index} 卷第 {ch_idx} 章 outline.goal 过抽象: {issue} "
                        f"(goal=「{goal_text[:60]}」) —— "
                        "下游 chapter_planner 会产抽象 scene_beats,"
                        "建议在 web UI 编辑此章 outline 加具体动作/对象,或重生本章"
                    ),
                )
    except Exception as _e:
        print(f"  ⚠ outline goal 具体度校验失败(不阻塞):{type(_e).__name__}: {_e}")
...
_OUTLINE_GOAL_ABSTRACT_LITERALS = {
    "推进剧情", "继续推进", "推动主线", "推动剧情", "推动故事",
    "深入了解", "进一步了解", "进一步深化", "深入探索",
    "增进感情", "巩固关系", "铺垫", "巩固",
    "继续发展", "推动节奏", "继续展开",
    "待定", "TODO", "todo", "TBD",
}

# 多次命中 → 抽象 (单次出现可以,但 ≥2 + 总长偏短就抽象)
_OUTLINE_GOAL_ABSTRACT_TOKENS = (
    "推进", "巩固", "铺垫", "深化", "了解", "深入", "继续", "进一步",
)


def _check_outline_goal_concreteness(goal: str) -> Optional[str]:
    """检查 outline.goal 具体度。返回 issue 描述,None=通过。

    规则:
    · 长度 < 15 字 → 抽象
    · 整句等于已知套话 → 抽象
    · 含 ≥2 个抽象 token 且总长 < 30 → 抽象
    """
    if not goal:
        return "goal 为空"
    s = goal.strip()
    if len(s) < 15:
        return f"过短 ({len(s)} 字 < 15,缺具体动作/对象)"
    if s in _OUTLINE_GOAL_ABSTRACT_LITERALS:
        return f"整句为抽象套话「{s}」"
    hit_tokens = [tok for tok in _OUTLINE_GOAL_ABSTRACT_TOKENS if tok in s]
    if len(hit_tokens) >= 2 and len(s) < 30:
        return (
            f"含 {len(hit_tokens)} 个抽象 token ({'/'.join(hit_tokens)}) "
            f"且总长 {len(s)} 字偏短"
        )
    return None

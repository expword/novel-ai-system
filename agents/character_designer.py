...
_SYSTEM_BASE = """你是专业的小说人物设计师，擅长创造有深度、关系复杂的人物体系。
【单主角铁律】
...
# ── 真实历史模式专属 SYSTEM addendum ─────────────────────
# 在 _SYSTEM_BASE 之上追加"必须用真实历史人物填主要角色 / 不许编造他们的核心言行"的硬约束
_SYSTEM_REAL_HISTORY_ADDENDUM = """

═══════════════════════════════════════════════════════════
🏛 本书故事根基 = 严格基于真实历史 —— 主要角色铁律
═══════════════════════════════════════════════════════════
【角色来源铁律（最高优先级）】
- 主角、主要配角、主要反派 **必须是真实历史人物**——优先从用户给出的"真实人物候选池"中选；
  池子不够用时，可补充该历史时期史载的其他重要人物（不许臆造同名人物）。
- 不许用真实朝代/时期的虚构姓名（如不许在唐贞观写"林惊羽""叶轻舟"这种网文式名字）。
- 卷内次要角色可以是虚构小人物（路人/仆从/无名将卒），但其姓名也要符合该时期取名规范。

【真实人物档案铁律】
- 真实历史人物的 personality / background / motivation / fatal_flaw / arc 必须扎根史料：
  · 性格 = 史载性格主线（如李世民"善纳谏、雄才大略、晚年多疑"），可以挖深心理，但不许颠倒
  · 背景 = 历史身世（出身、关键事件），不许虚构他们没经历过的家世
  · 动机 = 史载的核心抱负（如统一天下/匡扶汉室），可以补"内心戏"但不许换内核
  · arc = 必须符合史料中的人生轨迹（早年/中年/晚年走向、最终结局）
- realm 字段填史载身份（如"秦王/太子/天可汗"、"丞相"、"将军"）。
- relationships 必须忠于史载关系（如李世民与长孙皇后是夫妻、与魏征是君臣谏诤、与李建成是兄弟相残）。
- hidden_secret 可以挖史料未明的"内心戏/未公开心绪"——但不能与史实矛盾。

【is_real_person 标记】
- 每个人物的 JSON 必须带 "is_real_person": true/false 字段
- 真实历史人物填 true；虚构小人物填 false

不遵守 → canon_checker 会标 critical 触发返工。"""

_SYSTEM_REAL_ADAPTED_ADDENDUM = """

═══════════════════════════════════════════════════════════
📜 本书故事根基 = 真实人物/事件改编 —— 混合角色策略
═══════════════════════════════════════════════════════════
【角色来源策略】
- 主角可以是真实历史人物，也可以是虚构插入角色（穿越者/匿名小人物/史料未载者）。
  · 若主角是真实人物：按"真实人物档案铁律"设计（核心性格/动机/结局须忠于史料）
  · 若主角是虚构插入：他/她的故事在历史缝隙中展开，不可改写真实重大事件的结果
- 主要配角与主要反派 **必须包含至少 2-3 位真实历史人物**——优先从用户给出的"候选池"选；
  其他可以是为剧情服务的虚构角色。
- 真实历史人物的核心性格/历史立场/最终结局须忠于史料；中间过程的对话、心理、私下场景可文学化演绎。

【取名规范】
- 虚构角色的姓名必须符合该历史时期的命名习惯（如唐代人不叫"李夜羽"，民国人不叫"萧炎"）
- 不许把现代流行小说的网文式名字塞进历史背景

【is_real_person 标记】
- 每个人物的 JSON 必须带 "is_real_person": true/false 字段
- 真实历史人物填 true；为剧情虚构的角色填 false"""


def _get_system(state) -> str:
    """根据 state.creative_intent.reality_basis 拼接 SYSTEM——动态分支。

    - real_history → _SYSTEM_BASE + 严格历史 addendum
    - real_adapted → _SYSTEM_BASE + 混合改编 addendum
    - fictional / 未指定 → 仅 _SYSTEM_BASE
    """
    ci = getattr(state, "creative_intent", None)
    basis = ((ci.reality_basis if ci else "") or "").strip()
    if basis == "real_history":
        return _SYSTEM_BASE + _SYSTEM_REAL_HISTORY_ADDENDUM
    if basis == "real_adapted":
        return _SYSTEM_BASE + _SYSTEM_REAL_ADAPTED_ADDENDUM
    return _SYSTEM_BASE


# 向后兼容：原模块里 `from agents.character_designer import SYSTEM` 的调用点仍能跑
SYSTEM = _SYSTEM_BASE


def _format_real_persons_pool(state) -> str:
    """格式化"真实人物候选池"段——告诉 LLM 优先用名单里的人填主要槽位。
    ...
    if not ci:
        return ""
    basis = (ci.reality_basis or "").strip()
    if basis not in {"real_history", "real_adapted"}:
        return ""
    persons = [p.strip() for p in (ci.real_persons or []) if p and p.strip()]
    setting = (ci.historical_setting or "").strip()

    lines = ["═══ 真实人物候选池（优先从这里填主要角色）═══"]
    if setting:
        lines.append(f"  · 历史背景：{setting}")
    if persons:
        lines.append(f"  · 候选人物（{len(persons)} 位）：{' / '.join(persons)}")
        lines.append("  · 这些人物的台词/动机/抉择/结局必须符合史料；不许颠倒史载核心性格。")
    else:
        lines.append("  · （作者未列具体人物——请你根据上方历史背景，自行选择该时期史载的"
                       "重要人物来填主要角色槽位。）")
    if basis == "real_history":
        lines.append("  · 严格模式：主角 + 主要配角 + 主要反派必须**全部**是真实历史人物。")
    else:
        lines.append("  · 改编模式：主角可虚构，但主要配角/反派**必须包含至少 2-3 位**真实历史人物。")
    lines.append("  · 每个人物 JSON 必须带 \"is_real_person\": true/false。")
    return "\n".join(lines)
...
_CHAR_TEMPLATE = """
      ...
      "is_real_person": false,    // ⚠ 真实历史/改编模式下必填：true=真实历史人物（须忠于史料）; false=为剧情虚构。fictional 模式可省略或填 false
      "first_volume": 首次登场卷号,
...
"""
    data = request_json(
        system=_get_system(state), user=prompt,
...
def _flesh_out_slot(slot, common_ctx: str, protagonist_brief: str = "",
    ...
    system_prompt：调用方传入根据 state.creative_intent.reality_basis 拼好的 SYSTEM；
    留空时回退到通用 SYSTEM（向后兼容）。
    """
...
def _design_extended_cast(state: NovelState) -> None:
    """
    阶段B：每卷扩展角色——**卷间并发**(P0 优化:把原本 for 循环改为 parallel_map)。

    安全性:_generate_characters_batch 内部对 state.characters 的 append 在 GIL 下原子,
    多卷并发只会让"看到的 existing_cast_brief"是某一刻快照(可能略 stale),
    跨卷重名风险已经存在(同卷大批次时同样有),不是并发引入的新问题。

    优化前: 6 卷 × ~30s LLM = ~180s + 大卷拆批二次串行 ~90s = 总 ~270s
    优化后: 同时跑 6 卷,受 PARALLEL_WORKERS 限制 → 单卷耗时为整体耗时,约 60-90s
    """
    before_total = len(state.characters)

    # 每个 worker 处理一整卷(单批或多批由内部决定)
    def _design_one_volume(v):
        vi = v.index
    ...
    # 卷间并发(workers 数取 min(卷数, PARALLEL_WORKERS))
    workers = min(len(state.volumes) or 1, PARALLEL_WORKERS)
    print(f"  [P0 优化] 卷内角色 {len(state.volumes)} 卷并发(max_workers={workers})")
    parallel_map(
        fn=_design_one_volume,
        items=list(state.volumes),
        max_workers=workers,
        label="ExtendedCastByVolume",
    )
...
def _parse_and_add(state: NovelState, char_list: list) -> None:
    ...
    # 真实模式下，把 LLM 标了 is_real_person=true 的角色名 merge 回
    # state.creative_intent.real_persons —— 后续 canon_checker 就能盯防
    ci = getattr(state, "creative_intent", None)
    basis = ((ci.reality_basis if ci else "") or "").strip()
    is_real_mode = basis in {"real_history", "real_adapted"}
    if is_real_mode and ci and ci.real_persons is None:
        ci.real_persons = []
    ...
    for cd in char_list:
        ...
        if not name or name in existing_names:
            continue
        # ── 真实模式：把 is_real_person=True 的人名加入 real_persons ──
        if is_real_mode and ci and cd.get("is_real_person") is True:
            if name not in (ci.real_persons or []):
                ci.real_persons.append(name)
                # 有新真实人物入名单 → 自动开启 respect_real_figures
                ci.respect_real_figures = True
                state.memory.facts.append(
                    f"[真实历史人物-{name}] 由 character_designer 标记为 is_real_person=True，"
                    f"言行须符合史料；canon_checker 已纳入盯防名单。"
                )
        rels = [
            Relationship(

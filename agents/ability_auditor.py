...
- **代价是否在正文中显式描写**（cost_explicit=true/false）★ P1-1 硬要求 ★
  · true: 正文具体写出疲劳/损耗/被发现/伤害/物耗等代价描写
  · false: 正文未写出代价(或一笔带过"运气消耗"这种空话) → 记一个 no_cost issue
  · 不允许"代价存在但读者看不到"——读者看不到的代价等于没有
...
{{
  "ability_uses": [
    {{"ability_name": "...", "how_used": "...", "cost_paid": "...", "cost_explicit": true, "setting_match": true, "notes": ""}}
  ],
  "issues": [
        ...
        try:
            ...
            if abilities:
                ...
                for a in abilities[:10]:
                    ...
                    nodes = a.get("lifecycle_nodes") or []
                    node_brief = " → ".join(
                        f"C{n.get('target_chapter')}:{n.get('node_type')}"
                        for n in nodes[:8] if isinstance(n, dict) and n.get("target_chapter")
                    )
                    if node_brief:
                        fields.append(f"生命周期={node_brief}")
                    if fields:
                        detail_lines.append(f"· {a.get('name', '能力')}：" + "；".join(fields))
                if detail_lines:
                    parts.append("【特殊能力契约与时序】\n" + "\n".join(detail_lines))
        except Exception:
            pass
    ...
    *,
    ...
    for u in data.get("ability_uses", []) or []:
        ...
        uses.append(AbilityUse(
            ...
            cost_explicit=bool(u.get("cost_explicit", True)),
        ))
